...
static void GlobalBroadcast(struct dpu_set_t dpu_set,
    ...
    /* 确保共享段结束偏移是 8 字节对齐 */
    g_globalSectionEnd = ROUND_UP_TO_MULTIPLE_OF_8(
        g_commTotDeg_offset + ROUND_UP_TO_MULTIPLE_OF_8(commTot_bytes)
    );
    ...
    /* 调试输出 */
    // printf("[DEBUG] g_nodeCommunity_offset=%u (%%8=%u)\n", 
    //        g_nodeCommunity_offset, g_nodeCommunity_offset % 8);
    // printf("[DEBUG] g_commTotDeg_offset=%u (%%8=%u)\n", 
    //        g_commTotDeg_offset, g_commTotDeg_offset % 8);
    // printf("[DEBUG] g_globalSectionEnd=%u (%%8=%u)\n", 
    //        g_globalSectionEnd, g_globalSectionEnd % 8);
}
...
static void pushShardBroadcast(struct dpu_set_t dpu,
    ...
    sh->movedFlag_m = mram_heap_alloc(&alloc, 8);
    ...
    sh->P.twoM                       = twoM;
    /* 打印所有参数 */
    // printf("========== Shard Parameters ==========\n");
    // printf("numGlobalNodes             = %u\n", sh->P.numGlobalNodes);
    // printf("numLocalNodes              = %u\n", sh->P.numLocalNodes);
    // printf("numCommunities             = %u\n", sh->P.numCommunities);
    // printf("phase                      = %u (0=LocalMove, 1=Refinement)\n", sh->P.phase);
    // printf("movedFlag_m                = 0x%08X\n", sh->P.movedFlag_m);
    // printf("localNodeList_m            = 0x%08X\n", sh->P.localNodeList_m);
    // printf("localNodePtr_m             = 0x%08X\n", sh->P.localNodePtr_m);
    // printf("localNeighborIdxs_m        = 0x%08X\n", sh->P.localNeighborIdxs_m);
    // printf("nodeCommunity_m            = 0x%08X (global)\n", sh->P.nodeCommunity_m);
    // printf("communityTotalDegree_m     = 0x%08X (global)\n", sh->P.communityTotalDegree_m);
    // printf("activeNodes_m              = 0x%08X\n", sh->P.activeNodes_m);
    // printf("perCommunityComponentCount_m = 0x%08X\n", sh->P.perCommunityComponentCount_m);
    // printf("perNodeComponentIndex_m    = 0x%08X\n", sh->P.perNodeComponentIndex_m);
    // printf("managedCommStart           = %u\n", sh->P.managedCommStart);
    // printf("managedCommCount           = %u\n", sh->P.managedCommCount);
    // printf("twoM                       = %" PRIu64 "\n", sh->P.twoM);
    // printf("localNbrCount              = %u\n", sh->localNbrCount);
    // printf("======================================\n");
    ...
    if (sh->localNodeCount > 0) {
        ...
        if (perNodeComponentIndex_m) {
            /* 一次性传输：分配对齐的临时缓冲区并初始化为 0 */
            uint32_t *tmp_idx = (uint32_t*)calloc(sh->localNodeCount, sizeof(uint32_t));
            if (tmp_idx) {
                for (uint32_t i=0; i<sh->localNodeCount; ++i) tmp_idx[i] = 0xFFFFFFFFu;
                copyToDPU(dpu, (const uint8_t*)tmp_idx, perNodeComponentIndex_m,
                          (sh->localNodeCount)*sizeof(uint32_t));
                free(tmp_idx);
            }
        }
    ...
    uint32_t *tmp = (uint32_t*)calloc(C, sizeof(uint32_t));
    if (tmp) {
        for (uint32_t i=0; i<C; ++i) tmp[i] = 1;
        copyToDPU(dpu, (const uint8_t*)tmp, perCommunityComponentCount_m,
                    (C)*sizeof(uint32_t));
        free(tmp);
    }
