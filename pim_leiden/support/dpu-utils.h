...
static uint64_t load8B(uint32_t ptr_m, uint32_t idx, uint64_t* cache_w) {
    mram_read((__mram_ptr void const*)(DPU_MRAM_HEAP_POINTER + ptr_m + idx*sizeof(uint64_t)), cache_w, 8);
    return cache_w[0];
}
...
static uint32_t load4B(uint32_t ptr_m, uint32_t idx, uint64_t* cache_w) {
    ...
    mram_read((__mram_ptr void const*)(DPU_MRAM_HEAP_POINTER + ptr_block_m), cache_w, 8);
    // Extract 4B
...
static void store4B(uint32_t val, uint32_t ptr_m, uint32_t idx, uint64_t* cache_w) {
    ...
    // Write back 8B
    mram_write(cache_w, (__mram_ptr void*)(DPU_MRAM_HEAP_POINTER + ptr_block_m), 8);
}
